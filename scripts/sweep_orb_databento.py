...
def phase2_verdict(asset_key: str, params: dict, all_trial_sharpes: list[float]) -> tuple:
    ...
    16-year history, one-touch OOS, plus DSR: how much should the IS
    Sharpe of THIS finalist be deflated for having been picked as the
    best of `len(all_trial_sharpes)` grid combos tried for this asset?
    """
    cfg = ASSETS[asset_key]
    bars = list(load_bars_csv(cfg["data_path"]))
...
def main():
    ...
    print("DSR = P(true IS Sharpe > 0) after deflating for how many grid combos\n"
          ...
          "A finalist that's IS-significant on raw Sharpe but has low DSR is a\n"
          "likely case of 'best of N' noise, not real edge.\n")
    verdicts = []
    for ak in ASSETS:
        ...
        for r in screens[ak]["finalists"]:
            ...
            print(f"           DSR={dsr.dsr:.1%}  (n_trials={dsr.n_trials}, "
                  f"E[max Sharpe|noise]={dsr.expected_max_sharpe:+.2f}, "
                  f"IS Sharpe={dsr.sharpe:+.2f})", flush=True)
