...
ORB + MeanRev + IntradayMomentumTP x ES + NQ + GC.

v11: replaces the retired OD (OvernightDrift, disallowed -- see v10's
docstring) with IntradayMomentumTP (Gao/Han/Li/Zhou 2018 first-30-min-
momentum, symmetric 1:1 RR variant) as the ensemble's third leg. v10
(ORB+MeanRev only, no replacement) failed Gate 2/3 (t-stat 1.32 < 1.5).
IntradayMomentumTP evaluated SOLO also fails Gate 2 on every asset
(evaluate_intramom_databento.py) -- but so does ORB solo on the full
history, and ORB only clears Gate 2 once diversified into a multi-
stream ensemble. This tests whether IntradayMomentumTP plays the same
diversifying role OD used to, not whether it stands alone.

IntradayMomentumTP was picked over plain IntradayMomentum because the
screen (screen_new_daysession_strategies.py) showed it with a tighter
max_loss/avg_win ratio (symmetric 1:1 stop/target vs plain IntraMom's
wider 2x-signal-range stop with no matching target) -- the exact Gate 2
check IntraMom solo failed hardest. NQ and GC showed real IS edge
(pass30 30.3%/15.8%, Sharpe +0.53/+0.50); ES was weak/negative and is
included anyway, no cherry-picking.
...
(pass30 vs pass30, not $ vs $) as v1/v10. Also includes the sequential
(non-overlapping) account simulation and Monte Carlo block-bootstrap
pass-rate resample in one run.

Run with: python scripts/evaluate_ensemble_databento_v11_intramom.py
"""
...
IM_PARAMS  = dict(qty=1)  # pre-committed defaults, no tuning (screen used the same)
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v11 -- IntradayMomentumTP replaces retired OD")
    print("Strategies: ORB + MeanRev + IntradayMomentumTP x ES + NQ + GC")
    ...
    for ak in ASSETS:
        ...
        streams[(ak, "IntraMomTP")] = run_stream(
            bars, lambda tts=ts, a=ak: IntradayMomentumTP(
                symbol=a, tick_size=tts, **IM_PARAMS), ak)
