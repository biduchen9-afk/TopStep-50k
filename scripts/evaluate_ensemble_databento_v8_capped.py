"""9-stream regime-gated multi-asset ensemble, recent window -- v8: same
direct IS-pass30-maximizing search as v7, but with a hard per-stream
weight cap (MAX_WEIGHT=0.30) so the optimizer can't concentrate the
whole portfolio into one asset.

v7 (uncapped direct search) found IS pass30=42.0% -- by far the best of
any variant this session -- but concentrated 81% of weight into NQ
alone (NQ/ORB+NQ/MeanRev+NQ/OD), leaving ES+GC combined under 20%. Two
problems: (1) real single-asset concentration risk for live trading --
an NQ-specific bad stretch hits this portfolio far harder than a
balanced one; (2) OOS pass30 came back at only 26.2%, barely above v1's
27.7% despite the 42% IS number, suggesting the extra flexibility (8
free weights, direct optimization against one specific IS window) was
fitting IS-specific noise as much as durable structure.

v8 caps any single stream at 30% (water-filling redistribution of
anything over cap back onto the other streams, applied both inside the
optimizer's objective and to the final extracted weights) and re-runs
the identical direct pass30 search. This can't recover 100% of v7's
uncapped IS pass30 by construction -- the question is how much of it
survives the constraint, and whether a less concentrated portfolio
generalizes better OOS.

Compare against v1 (validated baseline, diversified pass-rate-normalized
weights), v6 (max-Sharpe, RETIRED at IS pass30=0.0%), and v7 (uncapped
direct search, IS pass30=42.0%, OOS pass30=26.2%, 81% NQ concentration).

Run with: python scripts/evaluate_ensemble_databento_v8_capped.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v8 -- CAPPED (<=30%/stream) IS-PASS30-MAXIMIZING WEIGHTS")
    ...
    # v7 (uncapped direct search) found IS pass30=42.0% -- but concentrated
    # 81% of weight into NQ alone (NQ/ORB+NQ/MeanRev+NQ/OD), and OOS pass30
    # came back at 26.2%, barely above v1's 27.7% despite the huge IS
    # number -- a sign the extra flexibility was fitting IS-specific noise
    # as much as real structure, on top of a real single-asset
    # concentration risk for live trading. MAX_WEIGHT caps any one stream
    # at 30%, forcing the search to keep some diversification across
    # assets rather than betting everything on whichever stream happened
    # to fit this IS window best.
    MAX_WEIGHT = 0.30
    ...
    def capped_weights(x: np.ndarray) -> np.ndarray:
        """softmax(x), then iteratively clip anything over MAX_WEIGHT and
        redistribute the excess proportionally among the uncapped
        components (water-filling), until stable."""
        w = softmax_weights(x)
        for _ in range(50):
            over = w > MAX_WEIGHT
            if not over.any():
    ...
    print(f"\n  Capped (max {MAX_WEIGHT:.0%}/stream) direct-search best IS pass30: {-res.fun:.1%} "
          f"(v1: 16.2%, v6 Sharpe-optimal: 0.0%, v7 uncapped: 42.0%)")
