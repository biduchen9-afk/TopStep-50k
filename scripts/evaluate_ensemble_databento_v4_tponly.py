...
weights -- v4: same idea as v3 (volatility-adaptive ORB, reusing the
rv5_prior/rv20_prior ratio that already drives orb_expansion_gate) but
with vol_scale_stop=False -- ONLY the take-profit widens/tightens with
the vol regime; the stop stays fixed at 40 ticks.

v3 (scaling both stop and TP together) came back OOS_PROMOTED but WORSE
than v1 on the metric that matters: OOS pass30 dropped from v1's 27.7%
to 21.2%, and the OOS MLL breach-rate advisory check FAILED (41.7% vs
22.6% IS, a 1.85x jump). The mechanism: wider stops on volatile days
raised average Sharpe/EV a bit but also raised the loss-per-trade on
the days it goes wrong, which is exactly what hurts pass30 (needs
CONSISTENCY, not just average edge). This variant targets that
specific failure mode directly: only the upside scales, the downside
stays capped at the original 40-tick stop.

Compare against v1 (validated baseline), v2 (unscaled 40t/40t, failed
Gate 3), and v3 (both-scaled, OOS_PROMOTED but worse pass30 than v1).

Run with: python scripts/evaluate_ensemble_databento_v4_tponly.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v4 -- ORB TP-ONLY SCALED BY rv5/rv20, STOP FIXED 40t")
    ...
    for ak in ASSETS:
        ...
        streams[(ak, "ORB")] = run_stream(
            bars, lambda gg=g, tts=ts, a=ak: OpeningRangeBreakout(
                ...
                vol_scale=gg["orb_scale"], vol_scale_stop=False, **ORB_PARAMS), ak)
