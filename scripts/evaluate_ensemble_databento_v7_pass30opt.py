"""9-stream regime-gated multi-asset ensemble, recent window -- v7: same
9 streams as v5/v6 (ORB=v1's validated shape; MeanRev/OD = per-asset
params from sweep_meanrev_od_databento.py), but weights come from
DIRECTLY maximizing IS pass30 via derivative-free search, instead of
proxying it through a smooth objective.

v6 (max-Sharpe mean-variance weights) came back with IS Sharpe=+2.10,
PF=1.46, MaxDD=only $1,488 -- and IS pass30 = 0.0%. Zero of 786 windows
passed. Sharpe-maximization aggressively minimizes variance, which built
an ultra-smooth portfolio (78% weight in the two MeanRev streams alone)
whose absolute dollar moves were too small to ever cross the fixed
+$3,000 target within 30 days, even though it also never went near the
-$2,000 floor. The Combine's pass criterion is a fixed-barrier problem
(reach +$3k before -$2k, within 30 days) -- not the same objective as
maximizing risk-adjusted return, and v6 proved they can point in
opposite directions. Every variant this session (v2-v6) implicitly
chased better Sharpe/EV as a pass30 proxy; that proxy just failed
outright.

v7 stops proxying: it searches the weight simplex (softmax
reparametrization, scipy differential_evolution -- pass30 is a
non-differentiable step function, so no gradient method applies) for
the combination that maximizes IS pass30 itself, scored via
realized_pass_rate directly on the blended series. Still IS-only, same
one-touch OOS discipline as every other variant.

Compare against v1 (validated OOS_PROMOTED baseline), v5 (same 9
streams, pass-rate-normalized weights, RETIRED at 12.2%), and v6
(same 9 streams, max-Sharpe weights, RETIRED at 0.0%).

Run with: python scripts/evaluate_ensemble_databento_v7_pass30opt.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v7 -- DIRECT IS-PASS30-MAXIMIZING WEIGHTS (v5 STREAMS)")
    ...
    w_opt = softmax_weights(res.x)
    print(f"\n  Direct-search best IS pass30 found: {-res.fun:.1%} "
          f"(vs v1's pass-rate-normalized 16.2%, v6's Sharpe-optimal 0.0%)")
