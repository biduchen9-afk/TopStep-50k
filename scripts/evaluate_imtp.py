"""Strategy #10: Intraday Momentum + Take-Profit (1:1 RR) — promotion checklist.

Pre-committed before any data is viewed:
  Predicted edge direction : MOMENTUM (continuation of first-30-min direction)
  Expected IS pass30 range : 10-25%
  Regime placement         : TRENDING / directional sessions
  IS trade freq expected   : ~100-130/year (same signal as #5)

  Key pre-committed difference from Strategy #5:
  - stop_multiple = 1.0 (1× signal range) vs 2.0 in #5
  - target_multiple = 1.0 (adds explicit take-profit at 1× signal range)
  - 1:1 RR should fix the max_loss/avg_win gate that #5 failed
  - win_rate ≥ 50% needed for positive EV (academic evidence: ~52-55%)

Parameters:
  signal_minutes = 30, min_signal_pct = 0.0010
  stop_multiple = 1.0, target_multiple = 1.0
  time_stop_minutes = 120, exit_time = 15:45

DSR accounting: +2 trials (raw + filtered) → cumulative ~40.
"""
...
def run_asset(asset_key: str, bars, with_filter: bool = False) -> dict[date, Decimal]:
    ...
    strat = IntradayMomentumTP(symbol=asset_key,
...
def main():
    print("=" * 72)
    print("STRATEGY #10: INTRADAY MOMENTUM + TAKE-PROFIT (1:1 RR)")
    print("Pre-committed: 30-min signal, stop=1x range, target=1x range")
    print("=" * 72)
    ...
    print(f"{'─'*72}")
    print_quick_stats("ES IMTP raw",    raw_full[is_mask],  is_days)
    print_quick_stats("ES IMTP +filter", filt_full[is_mask], is_days)

    print(f"\n{'─'*72}")
    print("GATE-BY-GATE CHECKLIST (raw variant)")
    print(f"{'─'*72}")
    result_raw = evaluate_strategy(
        label="IntradayMomTP-ES-raw",
        daily_full=raw_full,
    ...
    result_filt = evaluate_strategy(
        label="IntradayMomTP-ES-filtered",
    ...
    print(f"{'─'*72}")
    print_quick_stats("ES IMTP raw OOS",   raw_full[oos_mask],  oos_days)
    print_quick_stats("ES IMTP +filt OOS", filt_full[oos_mask], oos_days)

    print(f"\n{'─'*72}")
    print("DSR TRIAL ACCOUNTING")
    print(f"{'─'*72}")
    print("  Trials added this evaluation: 2 (raw + filtered)")
    print("  Cumulative project trials: ~38 + 2 = 40")
