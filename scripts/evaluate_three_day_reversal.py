"""Strategy #7: 3-Day Reversal — promotion checklist evaluation.

Pre-committed before any data is viewed:
  Predicted edge direction : CONTRA-TREND (long after 3-down, short after 3-up)
  Expected IS pass30 range : 10-25%
  Regime placement         : MEAN-REVERTING (avoid strong trending days)
  IS trade freq expected   : ~5-10/month (3 consecutive moves not rare in futures)

Parameters (pre-committed, literature-derived from Lehmann 1990):
  lookback      = 3   (3 consecutive closes in same direction)
  stop_atr_mult = 1.5 (1.5× 5-day ATR proxy)
  atr_lookback  = 5   (5 prior closes for ATR estimate)
  exit_time     = 15:45 ET (intraday only, flatten before close)

Evaluated on ES first. If OOS-promoted, fan out to NQ/GC.
DSR accounting: +2 trials (raw + filtered) → cumulative ~34.
"""
...
TDR_PARAMS = dict(qty=1, lookback=3, stop_atr_mult=1.5, atr_lookback=5)
...
def run_asset(asset_key: str, bars, with_filter: bool = False) -> dict[date, Decimal]:
    ...
    strat = ThreeDayReversal(symbol=asset_key,
...
def main():
    print("=" * 72)
    print("STRATEGY #7: 3-DAY REVERSAL — PROMOTION CHECKLIST")
    print("Pre-committed: long after 3-down closes / short after 3-up closes")
    print("=" * 72)
    ...
    print(f"{'─'*72}")
    print_quick_stats("ES ThreeDayReversal raw",    raw_full[is_mask],  is_days)
    print_quick_stats("ES ThreeDayReversal +filter", filt_full[is_mask], is_days)

    print(f"\n{'─'*72}")
    print("GATE-BY-GATE CHECKLIST (raw variant)")
    print(f"{'─'*72}")
    result_raw = evaluate_strategy(
        label="ThreeDayReversal-ES-raw",
        daily_full=raw_full,
    ...
    result_filt = evaluate_strategy(
        label="ThreeDayReversal-ES-filtered",
    ...
    print(f"{'─'*72}")
    print_quick_stats("ES ThreeDayRev raw OOS",   raw_full[oos_mask],  oos_days)
    print_quick_stats("ES ThreeDayRev +filt OOS", filt_full[oos_mask], oos_days)

    print(f"\n{'─'*72}")
    print("DSR TRIAL ACCOUNTING")
    print(f"{'─'*72}")
    print("  Trials added this evaluation: 2 (raw + filtered)")
    print("  Cumulative project trials: ~32 + 2 = 34")
