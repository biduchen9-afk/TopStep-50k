"""6-stream regime-gated multi-asset ensemble: ORB + MeanRev x ES + NQ + GC.

v10: OvernightDrift (OD) RETIRED. TopStep does not allow overnight or
weekend holding at ANY account stage (Combine, XFA, Live Funded) -- all
positions must be flat by 3:10 PM CT every day, confirmed current as of
a July 1, 2026 rules update (see docs/rules_sources.md). The prior v1
ensemble's OD_PARAMS streams carried ~43% of total weight (GC/OD=0.171
+ NQ/OD=0.154 + ES/OD=0.103) and were, per the DSR sweep, the single
most statistically significant component -- so this is a real test of
whether the surviving ORB+MeanRev combination still clears the
promotion bar on its own, not a cosmetic change.
...
(pass30 vs pass30, not $ vs $) as v1
(evaluate_ensemble_databento_recent_evgate.py) -- only the stream set
changed. Also includes the sequential (non-overlapping) account
simulation and Monte Carlo block-bootstrap pass-rate resample in one
run, since those are the numbers that actually matter for the >50%
threshold decision.

Run with: python scripts/evaluate_ensemble_databento_v10_no_overnight.py
"""
...
def main():
    print("=" * 72)
    print("6-STREAM ENSEMBLE v10 -- OD RETIRED (overnight holding not allowed)")
    print("Strategies: ORB + MeanRev x ES + NQ + GC")
    ...
    print("\nRunning 6 strategy-asset streams...", flush=True)
    ...
    for ak in ASSETS:
        ...
        print(f"  {ak}: 2 streams in {_time.time() - t0:.1f}s", flush=True)
