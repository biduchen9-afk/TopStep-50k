"""9-stream regime-gated multi-asset ensemble, recent window -- v6: same
9 streams as v5 (ORB=v1's validated shape; MeanRev/OD = per-asset params
from sweep_meanrev_od_databento.py), but replaces the pass-rate-
normalized weighting with a max-Sharpe mean-variance optimizer.

v5 improved three individual streams (ES/MeanRev, NQ/MeanRev, ES/OD all
better standalone Sharpe/pass30) yet the ENSEMBLE's IS pass30 fell from
v1's 16.2% to 12.2% and RETIRED. Mechanism: v1/v5's weighting normalizes
each stream by its OWN standalone IS pass30 -- diversification-blind by
construction. Improving NQ/MeanRev and ES/OD's solo scores pulled
allocation away from GC/OD, a stream with weak solo pass30 (11.1%) but
massive OOS $ (+$104k) whose return pattern happens to smooth the
blended curve; losing that hurt more than the individual gains helped.

v6 instead solves for the long-only weights that maximize the COMBINED
portfolio's own Sharpe ratio using the IS covariance matrix across
EV-positive streams (the standard mean-variance/tangency-portfolio
construction) -- still IS-only, same discipline, just a properly-posed
optimization instead of "normalize each stream's solo score."

Compare against v1 (validated OOS_PROMOTED baseline) and v5 (same 9
streams, old weighting, RETIRED).

Run with: python scripts/evaluate_ensemble_databento_v6_sharpeopt.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v6 -- MAX-SHARPE MEAN-VARIANCE WEIGHTS (v5 STREAMS)")
    ...
    # ── Per-stream IS stats + MEAN-VARIANCE (max-Sharpe) weights (IS only) ─
    # v1/v5 weighted each stream by its OWN standalone IS pass30, EV-gated.
    # That has no notion of how streams diversify TOGETHER: v5 improved
    # three individual streams (better own Sharpe/pass30 each) yet the
    # ENSEMBLE's pass30 fell, because the pass30-normalized weights pulled
    # allocation away from GC/OD -- a stream with weak standalone pass30
    # but a return pattern that happens to smooth the blended curve.
    # Naive per-stream scoring is diversification-blind by construction.
    #
    # This replaces that with the standard portfolio-construction answer:
    # solve for the long-only weights that maximize the COMBINED
    # portfolio's own Sharpe ratio (mean-variance / tangency portfolio),
    # using the IS covariance matrix across EV-positive streams. This is
    # still IS-only (no OOS touched to derive weights) -- same discipline
    # as before, just a better-posed optimization than "normalize each
    # stream's solo score."
    print(f"\n{'─'*72}")
    ...
    R = np.array([is_arrays[k] for k in included])  # (n_streams, n_days)
    mu = R.mean(axis=1)
    cov = np.cov(R)
    n = len(included)
    ...
    def neg_sharpe(w):
        ...
        if port_var <= 0:
            return 0.0
        return -(port_mean / np.sqrt(port_var)) * np.sqrt(252)
    ...
    if not res.success:
        print(f"  WARNING: Sharpe optimizer did not converge ({res.message}); "
              f"falling back to equal weights among EV-positive streams.")
        w_opt = x0
    else:
    ...
    for k, w in zip(included, w_opt):
        weights[k] = float(w)
    print(f"\nIS max-Sharpe mean-variance weights (EV-gated, long-only):")
