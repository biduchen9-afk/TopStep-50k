"""Full IS/OOS Gate 2/3/4 checklist for IntradayMomentum(TP), the one
survivor from screen_new_daysession_strategies.py's IS-only pass.

That screen found IntradayMomentumTP GATE2-OK on NQ (pass30=30.3%,
Sharpe=+0.53) and GC (pass30=15.8%, Sharpe=+0.50), plain IntradayMomentum
GATE2-OK on NQ (pass30=17.3%), and both weak/negative on ES. This runs
the real one-touch IS/OOS discipline (harness.evaluate_strategy) instead
of the light IS-only Gate2 proxy the screen used, on both variants,
across all 3 assets -- no cherry-picking just the good assets.

Run with: python scripts/evaluate_intramom_databento.py
"""
...
def run_cell(bars, asset_key, tick_size_f, instrument, build_fn):
    strat = build_fn(asset_key, tick_size_f)
...
def main():
    print("=" * 78)
    print("INTRADAY MOMENTUM (+TP variant) -- FULL IS/OOS 6-GATE CHECKLIST")
    print("=" * 78)
    ...
    for ak in ASSETS:
        ...
        for vname, build_fn in VARIANTS.items():
            t1 = _time.time()
            result = run_cell(bars, ak, cfg["tick_size_f"], cfg["instrument"], build_fn)
    ...
    for ak, vname, v in verdicts:
        print(f"  {ak}/{vname:<12}: {v.promoted:<14} IS-pass30={v.is_pass30:.1%} "
