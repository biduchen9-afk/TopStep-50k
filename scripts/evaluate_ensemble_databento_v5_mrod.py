...
weights -- v5: ORB kept at v1's validated shape (opposite_range stop,
1.0x-OR-width TP), but MeanRev and OD get per-asset parameters from
sweep_meanrev_od_databento.py's IS-only screen instead of one shared
config across all 3 assets. Neither strategy had been re-examined since
the timezone fix before that sweep.

Changes from v1: ES/MeanRev stop_ticks 15->30, time_stop 45->30 (sweep:
IS pass30 4.6%->6.7%, Sharpe 0.99->1.16). NQ/MeanRev lookback 60->90,
stop_ticks 15->20, time_stop 45->30 (IS pass30 1.4%->9.0%, Sharpe
0.13->1.30 -- was close to dead weight before). ES/OD entry_offset
5->15 (IS pass30 11.8%->18.1%). NQ/OD entry_offset 5->0 (small gain).
GC/MeanRev and GC/OD unchanged -- the sweep found no MeanRev config on
GC that wasn't Sharpe-negative, and GC/OD's original default was
already the sweep's #1 result.

Compare against v1 (evaluate_ensemble_databento_recent_evgate.py,
validated OOS_PROMOTED baseline).

Run with: python scripts/evaluate_ensemble_databento_v5_mrod.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v5 -- PER-ASSET MEANREV/OD PARAMS (SWEPT), ORB=v1 SHAPE")
