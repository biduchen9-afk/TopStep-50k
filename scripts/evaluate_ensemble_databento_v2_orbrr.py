...
recent window, EV-gated weights -- variant with the ORB leg swapped to
the winning shape from sweep_orb_rr_databento.py (fixed 40-tick stop,
40-tick TP, i.e. 1:1 R:R) instead of the original opposite_range stop /
1.0x-OR-width TP. MeanRev and OD parameters are unchanged (not yet
re-tuned on the timezone-corrected data).

Compare directly against evaluate_ensemble_databento_recent_evgate.py
(same everything, old ORB shape) to see whether the better standalone
ORB shape also lifts the diversified ensemble.

Run with: python scripts/evaluate_ensemble_databento_v2_orbrr.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE v2 -- ORB SWAPPED TO R:R-SWEEP WINNING SHAPE (40t/40t)")
