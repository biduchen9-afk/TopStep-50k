"""9-stream regime-gated multi-asset ensemble (ORB+MeanRev+OD x ES+NQ+GC),
re-evaluated on the new Databento dataset (2010-06 -> 2026-07, ~16 years)
instead of the old *_cleaned.txt files (~4 years).

Same strategies, same pre-committed parameters, same causal regime gates,
same 70/30 IS/OOS harness as evaluate_ensemble_is.py + evaluate_ensemble_oos.py
-- this is a straight re-run on more/different history, not a re-fit.
Single strategy-only ORB (both raw and vol-expansion-gated) already failed
Gate 2 on this longer history (see sweep_orb_databento.py and
sweep_orb_gated_databento.py); this checks whether the full 9-stream
combination -- which is what was actually OOS_PROMOTED before -- still
clears the bar once diversified across 3 strategies x 3 assets.

Run with: python scripts/evaluate_ensemble_databento.py
"""
...
def main():
    print("=" * 72)
    print("9-STREAM ENSEMBLE -- DATABENTO DATASET, RECENT WINDOW, EV-GATED WEIGHTS")
